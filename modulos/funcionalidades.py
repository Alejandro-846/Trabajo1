        ...
        case 1:
            for campers, camper in base['Campers'].items():
                if camper["numIdentificacion"] == contraseña and campers == usuario:
                    ...
                    ctrl.pausar_pantalla()
                    return(1)
            print(f'Usuario y/o Contraseña no coniciden o no existen')
            ctrl.pausar_pantalla()
            return(4)
        case 2:
        ...
        case 'Cursando':
            ...
            for camperNum, dirCamper in base['Campers'].items():
                if dirCamper['numIdentificacion'] == id:
                    ...
                    sumaNotasQT= notaQuisLista.sum(notaQuisLista)
                    cantidadNotasQT=notaQuisLista.len()
                    ...
                    notaMod:int = (*0.1)+(notaProyecto*0.6)+(notaExamen*0.3)
                    dirCamper['notas'][modulo]["notaModulo"].append(notaMod)
        case _:
            print(f'El estado del camper {camper} es: {estado} no se puede trabajar sus notas')
            ctrl.pausar_pantalla()
